...
public class MyList<Integer> extends ArrayList<Integer> {
    ...
    public boolean addAll(Collection<? extends Integer> c) {
        /*for(Integer element : c){
            if(Collections.frequency(c,element)==1 && !this.contains(element)){
                differentElements++;
            }
        }*/

        ArrayList<Integer> myList = new ArrayList<Integer>(c);

        for(int i = 0;i < myList.size(); i++){
            if(myList.indexOf(myList.get(i)) == i &&
                    myList.lastIndexOf(myList.get(i)) == i){
                if(!this.contains(myList.get(i))){
    ...
    public Integer remove(int index) {
        if(this.indexOf(this.get(index)) == index &&
                this.lastIndexOf(this.get(index)) == index)
            this.differentElements--;
